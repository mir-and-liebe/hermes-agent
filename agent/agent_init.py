...
def init_agent(
    ...
    agent._compression_checkpoint_enabled = compression_checkpoint_enabled
    agent._compression_checkpoint_dir = compression_checkpoint_dir
    agent._last_compression_checkpoint_path = None
    agent._context_pressure_bands = resolve_context_pressure_bands(
        context_length=getattr(agent.context_compressor, "context_length", 0) or 1,
        config=_compression_cfg,
    )
    agent._last_context_pressure_band = "normal"
    if hasattr(agent.context_compressor, "threshold_tokens"):
        agent.context_compressor.threshold_tokens = max(
            agent._context_pressure_bands.normal_compaction_tokens,
            MINIMUM_CONTEXT_LENGTH,
        )
    if hasattr(agent.context_compressor, "threshold_percent") and agent._context_pressure_bands.context_length:
        agent.context_compressor.threshold_percent = (
            agent.context_compressor.threshold_tokens
            / agent._context_pressure_bands.context_length
        )
    if hasattr(agent.context_compressor, "summary_target_ratio") and hasattr(agent.context_compressor, "tail_token_budget"):
