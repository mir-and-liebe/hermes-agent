...
from agent.context_pressure import classify_context_pressure
...
def _ra():
    """Lazy reference to run_agent so tests patching run_agent symbols still work."""
    import run_agent
    return run_agent
...
def compress_context(
    ...
    *,
    ...
    if getattr(agent, "_compression_checkpoint_enabled", True):
        try:
            _bands = getattr(agent, "_context_pressure_bands", None)
            _pressure = (
            ...
            checkpoint_path = _ra().write_context_checkpoint(
                hermes_home=_ra().get_hermes_home(),
                checkpoint_dir=getattr(agent, "_compression_checkpoint_dir", "context-checkpoints"),
                session_id=agent.session_id or "unknown",
                reason=f"{_checkpoint_reason}_compaction",
                content=_checkpoint_content,
            )
            agent._last_compression_checkpoint_path = str(checkpoint_path)
            _test_hook = getattr(agent, "_on_compression_checkpoint_written_for_test", None)
