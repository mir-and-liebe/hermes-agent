    ...
    for entry in worktrees_dir.iterdir():
        ...
        if lock_state == "dead":
            try:
                ...
                logger.debug("Unlocked dead-locked worktree: %s", entry.name)
            except Exception as e:
                logger.debug("Failed to unlock dead-locked worktree %s: %s", entry.name, e)
                continue  # Still locked — can't remove
        ...
        try:
            ...
            for line in branch_result.stdout.splitlines():
                if line.startswith("worktree ") and str(entry) in line:
                    for branch_line in branch_result.stdout.splitlines():
                        if branch_line.startswith("branch "):
                            branch = branch_line.split(" ", 1)[1]
                            break
            ...
            # Clean up the branch if removal succeeded
            if branch and (branch.startswith("hermes/") or branch.startswith("pr-")):
                subprocess.run(
                    ["git", "branch", "-D", branch],
                    capture_output=True, text=True, timeout=10, cwd=repo_root,
                )
